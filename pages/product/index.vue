    ...
    <view class="page">
		<view class="head">
			<view class="box1 flex-center-between">
				<view class="search flex-center-start">
					...
					<input v-model="jijin_name" @confirm="confirm()" placeholder-style="color: #C1C1C1;font-size: 26rpx;" style="width: 100%;" placeholder="基金名称" />
				</view>
		...
		<view class="big_box">
			<view class="head_btn flex-center-around">
				<navigator url="./all_fund" hover-class="none" class="item">
					<image class="icon" src="../../static/product/qbjj.png"></image>
					<view class="text">全部基金</view>
				</navigator>
				...
				<navigator url="./new_fund" hover-class="none" class="item">
					<image class="icon" src="../../static/product/xfjj.png"></image>
					<view class="text">新发基金</view>
				</navigator>
			</view>
			<swiper class="swiper_box swiper" indicator-color="#CBCBCD" indicator-active-color="#004EA3" :indicator-dots="true" :autoplay="true" :interval="4000" :duration="500">
			    <swiper-item v-for="(row,i) in product1" :key="i">
			        <view class="content">
			        	...
			        	<view class="flex-center-center">
			        		<view @click="jump('./info?id=' + row.id)" class="flex-center-center btn">立即购买</view>
			        	</view>
			        </view>
			    </swiper-item>
			</swiper>
		<!-- 	<view class="title flex-center-between" style="margin-top: 20rpx;">
				<view>热度排行</view>
				<navigator url="./hot_fund" hover-class="none" class="more">更多</navigator>
			</view>
			<view class="box1">
				<view @click="jump('./info?id=' + row.id)" class="item flex-center-start" v-for="(row,i) in product2" :key="i">
					<image class="icon" :src="helper.uploadUrl + row.pic"></image>
					<view class="flex_1">
						<view class="flex-center-between" style="margin-bottom: 12rpx;">
							<view class="text1">{{row.title}}</view>
							<view class="text2">{{row.buy_count}}</view>
						</view>
						<view class="flex-center-between">
							<view class="text3">{{row.describe}}</view>
							<view class="text4">最近购买次数</view>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="box2 flex-center-around">
				<view  @click="jump('./info?id=' + row.id)" class="item" v-for="(row,i) in product3" :key="i">
					<view style="padding-left: 14rpx;">
						<view class="text1">{{row.title}}</view>
						<view class="text2">{{row.genre}}</view>
						<view class="text3">{{row.describe}}</view>
						<view class="text4">{{row.jyrsy}}%</view>
						<view class="text5">日收益</view>
					</view>
				</view>
			</view> -->
			<view class="title flex-center-between" style="margin-top: 20rpx;">
		...
		<view class="all_fund">
			...
			<view class="item" v-for="(row,i) in list" :key="i">
				...
				<view class="item_box flex-center-between">
					...
					<view>
						<view class="text3">{{row.mrsy}}</view>
